...
void updateTreeD(Node* &root, Color::Color u, Color::Color v, Node* parent, bool isLeft){
  ...
  if(v == BLACK && u == BLACK){
    ...
    p == NULL ? cout << "Parent is null" << endl: cout << "Parent is " << p->getColor() << " with a value of " << parent->getData()<< endl;
	...
	if(s == p->getL()){
	  //ll
	  s->getL()->setColor(s->getColor());
	  s->setColor(p->getColor());
	  rotateRight(root, p);
	}else{
	  //rl
	  s->getL()->setColor(p->getColor());
	  rotateRight(root, s);
	  rotateLeft(root, p);
	}
      }else{
	if(s == p->getL()){
	  //lr
	  s->getR()->setColor(p->getColor());
	  rotateLeft(root, s);
	  rotateRight(root, p);
	}else{
	  //rr
	  s->getR()->setColor(s->getColor());
	  s->setColor(p->getColor());
	  rotateLeft(root, p);
	}
      }
